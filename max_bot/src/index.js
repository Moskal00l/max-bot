...
bot.command("help", async (ctx) => {
  ...
  try {
    const image = await ctx.api.uploadImage({
      url: "https://upload.wikimedia.org/wikipedia/commons/4/43/BOThelp.png",
    });
    await bot.api.sendMessageToChat(
      ...
      "📋 **Основные возможности**\n" +
        ...
        "📊 Отметка участников по QR-коду",
      { format: "markdown", attachments: [image.toJson()] }
    );
  } catch (err) {
    console.log(err);
    await ctx.reply(
      "К сожалению, случились технические-шоколадки. Попробуйте позже."
    );
  }
});
...
bot.on("bot_started", async (ctx) => {
  ...
  try {
    ...
    ctx.reply(
      "К сожалению, данная ссылка недействительна.\nПопросите у организатора другую ссылку."
    );
...
bot.action(/register:(.+)/, async (ctx) => {
  ...
  const avatar = data[1];
  const user = {
    user_id: ctx.update.callback.user.user_id,
  ...
  const keyboard_app = Keyboard.inlineKeyboard([
    [
      Keyboard.button.link(
        "Открыть приложение",
        "https://max.ru/t159_hakaton_bot?startapp"
      ),
    ],
  ]);

  await bot.api.editMessage(message_id, {
    text: message_text,
    attachments: [],
  });

  await bot.api.sendMessageToChat(id_chat, "**Вы зарегистрированы ✅**", {
    format: "markdown",
  ...
  await bot.api.sendMessageToUser(
    ...
    "С радостью сообщаем 🪄:\n\n" +
      ...
      " " +
      user.last_name +
      'стал участником вашего мероприятия "' +
      event_title +
      '"!',
    { format: "markdown" }
  );
